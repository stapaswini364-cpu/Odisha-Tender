...
export const tenders = pgTable(
  ...
  (table) => ({
    externalKeyUnique: uniqueIndex("tenders_external_key_unique").on(
      table.externalKey
    ),
  })
);
...
export const jobRuns = pgTable("job_runs", {
  ...
  // running / success / failed / captcha_failed
...
// Tracks batch progress / cursor across cron runs
export const scraperState = pgTable(
  "scraper_state",
  {
    id: uuid("id").defaultRandom().primaryKey(),
...
// 7. Cron lock table
// Prevents multiple check-tenders cron jobs from running simultaneously
