...
async function main() {
  ...
  try {
    ...
    // Step 2: Process each organisation
    for (let i = 0; i < organisations.length; i++) {
      ...
      try {
        ...
        const tenders = await scrapeTendersByOrganisation(
          organisation.name
        );

        console.log(`Scraped: ${tenders.length}`);

        // Step 4: No tenders found
        if (tenders.length === 0) {
          console.log("No tenders found.");
          continue;
        }

        // Step 5: Insert / skip duplicate tenders
    ...
    // Step 8: Show failed organisations
    if (failedOrganisations.length > 0) {
      console.log("\n========================================");
      console.log("       FAILED ORGANISATIONS");
      console.log("========================================");

      failedOrganisations.forEach((item, index) => {
        console.log(
          `\n${index + 1}. ${item.name}`
        );
